...
void UCharacterAnimInstance::NativeUpdateAnimation(float DeltaSeconds)
{
	...
	if(bWeaponEquipped && EquippedWeapon && EquippedWeapon->GetWeaponMesh() && PlayerRef->GetMesh())
	{
		...
		if(PlayerRef->IsLocallyControlled())
		{
			...
			FRotator LookAtRotation = UKismetMathLibrary::FindLookAtRotation(RightHandTransform.GetLocation(), RightHandTransform.GetLocation() + (RightHandTransform.GetLocation() - PlayerRef->GetHitTarget()));
			RightHandRotation = FMath::RInterpTo(RightHandRotation, LookAtRotation, DeltaSeconds, 30.f);
		}
	}

	bUseFABRIK = PlayerRef->GetCombatState() == ECombatState::ECS_Unoccupied;
	bUseAimOffsets = PlayerRef->GetCombatState() == ECombatState::ECS_Unoccupied && !PlayerRef->GetDisableGameplay();
	bTransformRightHand = PlayerRef->GetCombatState() == ECombatState::ECS_Unoccupied && !PlayerRef->GetDisableGameplay();
}
